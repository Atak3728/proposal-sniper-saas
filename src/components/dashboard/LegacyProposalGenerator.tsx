...
const LegacyProposalGenerator = ({ initialJobDescription = '', variant = 'default', applicationId, initialContent }: { initialJobDescription?: string, variant?: 'default' | 'integrated', applicationId?: string, initialContent?: string }) => {
    const { user } = useUser();
    ...
    const [prompt, setPrompt] = useState(initialJobDescription);

    // New State for Model
    const [model, setModel] = useState('GPT-4o');

    // State for the AI Output - Initialize with saved content if available
    const [completion, setCompletion] = useState(initialContent || '');
    ...
    // Update prompt if initialJobDescription changes (active/inactive state or navigation)
    useEffect(() => {
        if (initialJobDescription) {
            setPrompt(initialJobDescription);
        }
    ...
    useEffect(() => {
        ...
        if (outputDraft && !initialContent) { // Only load local draft if no DB content
    ...
    const handleSave = async (contentToSave: string) => {
        if (applicationId) {
            // Import dynamically to avoid server/client issues or ensure it is a server action suitable for client call
            const { saveProposal } = await import('@/actions/proposal-actions');
            await saveProposal(applicationId, contentToSave);
        }
    };
    ...
    const generateProposal = async () => {
        ...
        try {
            ...
            // Save to DB (Legacy History)
            if (user?.id) {
                await saveProposal(user.id, prompt, fullContent);
            }

            // Save to Application DB (New Persistence)
    ...
    if (variant === 'integrated') {
        ...
        if (!hasResult) {
            return (
                <div className="flex flex-col h-full w-full bg-white dark:bg-[#13151C] relative font-display">
                    <div className="flex flex-col items-center justify-center h-full text-center p-8">
                        <div className="w-20 h-20 rounded-full bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center mb-6">
                            <span className="material-symbols-outlined text-indigo-500 text-4xl">edit_document</span>
                        </div>
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Freelance Proposal Generator</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm mb-8">
                            Craft a winning proposal based on the job requirements.
                        </p>
                        ...
                        <button
                            ...
                            disabled={!prompt}
                            className="bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-6 rounded-xl shadow-lg shadow-indigo-500/20 hover:scale-105 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            <div className="flex flex-col h-full w-full bg-white dark:bg-[#13151C] relative font-display">
                ...
                <div className="h-14 border-b border-gray-100 dark:border-white/5 flex items-center justify-between px-4 bg-gray-50/50 dark:bg-[#1c1f26]/50">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={() => {
                                handleSave(completion); // Manual Save
                                setToastMessage('Saved!');
                                setShowToast(true);
                            }}
                            className="p-1.5 rounded-md text-gray-400 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-500/10 transition-colors"
                            title="Save"
                        >
                        ...
                        <button
                            onClick={() => {
                                setCompletion('');
                                // Optionally clear prompt or keep it? Keeping prompt allows re-generating with different settings.
                            }}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center h-full gap-4">
                            ...
                            <p className="text-sm font-medium text-gray-500 animate-pulse">Drafting proposal...</p>
                        </div>
                    ) : (
                        <div className="prose prose-sm max-w-none dark:prose-invert font-medium text-gray-900 dark:text-gray-200 leading-relaxed whitespace-pre-wrap">
...
export default LegacyProposalGenerator;
